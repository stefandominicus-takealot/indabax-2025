...
def _get_feature_cardinality(
    ...
    """
    Calculate feature cardinality (the TFTransformOutput vocabulary size doesn't account
    for Out Of Vocabulary (OOV) buckets).
    """
...
def _get_numerical_feature_encoder(feature_name: str, output_dim: int) -> keras.Model:
    return keras.Sequential(
        layers=[
            keras.layers.Dense(
...
def _get_text_feature_encoder(feature_name: str, output_dim: int) -> keras.Model:
    """
    If text features are transformed (see `transform_module.py`) into embeddings using a
    pre-trained language model (e.g., BERT), then this function needs to use the correct
    `input_shape` below.
    """
    return keras.Sequential(
        layers=[
...
class CustomerTower(keras.Model):
    def __init__(self, tf_transform_output: TFTransformOutput):
        ...
        # TODO[IndabaX]: Consider adding a dropout layer
        # TODO[IndabaX]: Consider adding hidden dense layers
...
class ProductTower(keras.Model):
    def __init__(self, tf_transform_output: TFTransformOutput):
        ...
        self._feature_encoders = {
            ...
            # TODO[IndabaX]: Consider adding other product features
        }
        self._common = keras.Sequential(
            layers=[keras.layers.Concatenate()], name="common"
        )
    ...
    def compute_loss(
        ...
        if self._ranking_task:
            raise NotImplementedError(
                "Calculating a ranking loss is not yet implemented."
            )
