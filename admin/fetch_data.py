...
QUERY = f"""WITH
    ...
    WHERE
        ...
        AND DATE(updated_at) BETWEEN "2025-01-01" AND "2025-07-01" -- RECENT
    GROUP BY
        uuid ),
    customers AS (
    SELECT
...
if __name__ == "__main__":
    ...
    # Strip the TEST split so that it remains unseen
    reviews_df = reviews_df[reviews_df[SPLIT] != Splits.TEST].drop(columns=[SPLIT])
